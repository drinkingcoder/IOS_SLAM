...
#define MEDIA_ARRAY_SIZE 50
#define MOTION_ARRAY_SIZE 100
...
@property(nonatomic, retain) NSOperationQueue *gyroQueue;
...
- (void)start
{
	if ( !self.motionManager.deviceMotionActive ) {
		...
		if ( self.motionManager.deviceMotionAvailable ) {
            ...
            [self.motionManager startAccelerometerUpdatesToQueue:self.accelerometerQueue withHandler:^(CMAccelerometerData* accData,NSError *error) {
               // NSLog(@"Call back");
                if(!error)
                    [_delegate updateAccelerometerData:accData];
            }];
            [self.motionManager startGyroUpdatesToQueue:self.gyroQueue withHandler:^(CMGyroData* gyroData, NSError *error){
                if(!error)
                    [_delegate updateGyroData:gyroData];
            }];
		}
	}
}
...
- (void)stop
{
	if ( self.motionManager.deviceMotionActive ) {
        ...
        [self.accelerometerQueue addOperationWithBlock:^{
            @synchronized(self) {
                [self.motionSamples removeAllObjects];
            }
        }];
        [self.gyroQueue addOperationWithBlock:^{
            @synchronized(self) {
                [self.motionSamples removeAllObjects];
            }
        }];
...
- (void)setMotionRate:(int)motionRate
{
    ...
    updateIntervalSeconds = 1.0/AccelerometerUpdateRate;
    self.motionManager.accelerometerUpdateInterval = updateIntervalSeconds;
    updateIntervalSeconds = 1.0/AccelerometerUpdateRate;
    self.motionManager.gyroUpdateInterval = updateIntervalSeconds;
