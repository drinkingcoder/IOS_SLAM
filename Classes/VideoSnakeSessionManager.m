...
@interface VideoSnakeSessionManager () <AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, MovieRecorderDelegate, MotionSynchronizationDelegate>
{
    ...
    dispatch_queue_t _uploadSyncedQueue;
    dispatch_queue_t _saveDataSyncedQueue;
    dispatch_semaphore_t uploadThreadSemaphore;
    dispatch_group_t uploadThreadGroup;
    
    NSMutableArray *imageTimeStampArray;
    NSMutableArray *accelerometerDataArray;
    NSMutableArray *gyroDataArray;
...
- (id)init
{
	if (self = [super init]) {
        ...
        _uploadSyncedQueue = dispatch_queue_create("com.apple.sample.sessionmanager.upload", DISPATCH_QUEUE_SERIAL);
        _saveDataSyncedQueue = dispatch_queue_create("com.apple.sample.sessionmanager.saveData",DISPATCH_QUEUE_SERIAL);
        uploadThreadSemaphore = dispatch_semaphore_create(20);
        uploadThreadGroup = dispatch_group_create();
        ...
        imageTimeStampArray = [[NSMutableArray alloc] init];
        accelerometerDataArray = [[NSMutableArray alloc] init];
        gyroDataArray = [[NSMutableArray alloc] init];
        
        networkSession = [AFHTTPSessionManager manager];
        networkSession.responseSerializer = [AFHTTPResponseSerializer serializer];
        networkSession.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json",
                                                             //   @"text/html",
                                                             @"image/jpeg",
                                                             @"image/png",
...
- (void)dealloc
{
    ...
    if( _uploadSyncedQueue )
        [_uploadSyncedQueue release];
    
    if( _saveDataSyncedQueue)
        [_saveDataSyncedQueue release];
...
- (void)setupCaptureSession
{
    ...
    /*
    [videoOut setVideoSettings:[NSDictionary dictionaryWithObjects:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange]
...
-(void) upload:(NSData*) imageData
 withTimeStamp:(double)imageTimeStamp
withAccelerometerData:(NSMutableArray*)accData
  withGyroData:(NSMutableArray*)gyroData
    withNumber:(int)num
{
    dispatch_semaphore_wait(uploadThreadSemaphore, DISPATCH_TIME_FOREVER);

    [networkSession POST:kURLString
       parameters:nil
constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formdata)
     {
         NSString *fileName = [NSString stringWithFormat:@"image%d",num];
         [formdata appendPartWithFileData:imageData
                                     name:fileName
                                 fileName:fileName
                                 mimeType:@"image/png"];
         //dispatch_group_enter(uploadThreadGroup);
     ...
     {
        // NSLog(@"Success!");
         //dispatch_group_leave(uploadThreadGroup);
         dispatch_semaphore_signal(uploadThreadSemaphore);
         // NSLog(@"%@",responseObject);
     }
          failure:^(NSURLSessionDataTask* _Nullable task,
                    NSError * _Nonnull error)
     {
         NSLog(@"%d upload Failed!",num);
         NSLog(@"%@",error);
         //dispatch_group_leave(uploadThreadGroup);
         dispatch_semaphore_signal(uploadThreadSemaphore);
     }
...
constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formdata)
     {
         ...
         [dataString appendFormat:@"%lf\n",imageTimeStamp];
         
         [dataString appendFormat:@"%lu\n",(unsigned long)[accData count]];
         for(int i=0;i<[accData count];i++)
             [dataString appendFormat:@"%@\n%@ %@ %@\n",accData[i][0],accData[i][1],accData[i][2],accData[i][3]];
         
         [dataString appendFormat:@"%lu\n",(unsigned long)[gyroData count]];
         ...
         NSString *fileName = [NSString stringWithFormat:@"motionData%d",num];
...
- (NSRange)getAccelerometerRangeStartTimeStamp:(NSNumber*)startTimeStamp
                          endTimeStamp:(NSNumber*)endTimeStamp
{
    unsigned long startIndex = 0,len = [accelerometerDataArray count];
    NSArray *data;
    for(int i=0;i<[accelerometerDataArray count];i++)
    {
        //NSLog(@"%@",accelerometerDataArray[i]);
        data = accelerometerDataArray[i];
        if(startTimeStamp.doubleValue > ((NSNumber*)data[0]).doubleValue) startIndex = i;
        if(((NSNumber*)data[0]).doubleValue > endTimeStamp.doubleValue)
        {
            len = i - startIndex;
            break;
        }
...
- (NSRange)getGyroRangeStartTimeStamp:(NSNumber*)startTimeStamp
                                  endTimeStamp:(NSNumber*)endTimeStamp
{
    unsigned long startIndex = 0,len = [gyroDataArray count];
    NSArray *data;
    for(int i=0;i<[gyroDataArray count];i++)
    {
        NSLog(@"%@",gyroDataArray[i]);
        data = gyroDataArray[i];
        if(startTimeStamp.doubleValue > ((NSNumber*)data[0]).doubleValue) startIndex = i;
        if(((NSNumber*)data[0]).doubleValue > endTimeStamp.doubleValue)
        {
            len = i - startIndex;
...
{
    NSLog(@"upload %lu images",(unsigned long)[imageTimeStampArray count]);
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES);
    static NSNumber* lastImageTimeStamp, *currentImageTimeStamp;
    NSLog(@"acc count = %lu",(unsigned long)[accelerometerDataArray count]);
    NSLog(@"gyro count = %lu",(unsigned long)[gyroDataArray count]);
    for(int i=0;i<[imageTimeStampArray count];i++)
    {
        NSString *filePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:[NSString stringWithFormat:@"image%d", i]];
        NSData * mydata = [NSData dataWithContentsOfFile:filePath];
        currentImageTimeStamp = imageTimeStampArray[i];
        ...
        if(i!=0)
        {
            NSRange accRange = [self getAccelerometerRangeStartTimeStamp:lastImageTimeStamp endTimeStamp:currentImageTimeStamp];
            NSRange gyroRange = [self getGyroRangeStartTimeStamp:lastImageTimeStamp endTimeStamp:currentImageTimeStamp];
            NSLog(@"location = %lu, len = %lu",(unsigned long)accRange.location,accRange.length);
            for(unsigned long index = accRange.location;index<accRange.location+accRange.length;index++)
            {
                NSLog(@"%lu",index);
                [accData addObject:accelerometerDataArray[index]];
            }
            for(unsigned long index = gyroRange.location;index<gyroRange.location+gyroRange.length;index++)
                [gyroData addObject:gyroDataArray[index]];
        }
        
        [self upload:mydata withTimeStamp:currentImageTimeStamp.doubleValue withAccelerometerData:accData withGyroData:gyroData withNumber:i];
        
        lastImageTimeStamp = imageTimeStampArray[i];
        
        [self deleteFile:filePath];
        [accData release];
        [gyroData release];
    }
    [accelerometerDataArray removeAllObjects];
    [gyroDataArray removeAllObjects];
    
    NSLog(@"uploaded all images");
}
...
- (void)motionSynchronizer:(MotionSynchronizer *)synchronizer didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer withMotion:(CMDeviceMotion *)motion
{
    @synchronized (self) {
        if( self.catchImage )
        {
            ...
            dispatch_async(_saveDataSyncedQueue, ^{
                ...
                uint8_t *yPlane = CVPixelBufferGetBaseAddress(imageBuffer);
                static uint8_t yBuffer[640*480*4];
                
                for(int i=0;i<height;i++)
                {
                    for(int j=0;j<width;j++)
                    {
                        yBuffer[i*width+j] = (yPlane[i*width*4+j*4] + yPlane[i*width*4+j*4+1] + yPlane[i*width*4+j*4+2])/3;
                    }
                }
                
                NSData* imageData = [NSData dataWithBytes:yBuffer length:height*width];
                
                NSString* imageName = [NSString stringWithFormat:@"image%lu",(unsigned long)[imageTimeStampArray count]];
                NSString* documentPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
                NSString* avatarPath = [documentPath stringByAppendingPathComponent:imageName];
                [imageData writeToFile:avatarPath atomically:YES];
                // [imageUploadArray addObject:imageData];
                //double motionTimeStamp = [motion timestamp];
                double imageTimeStamp = CMTimeGetSeconds(CMSampleBufferGetPresentationTimeStamp(sampleBuffer));
                [imageTimeStampArray addObject:[NSNumber numberWithDouble:imageTimeStamp]];
...
{
    if(!self.catchImage) return;
    NSLog(@"update accelerometerData %@",data);
    @autoreleasepool {
    NSArray *newData = [NSArray arrayWithObjects:
                        [NSNumber numberWithDouble:data.timestamp],
                        [NSNumber numberWithDouble:data.acceleration.x],
                        [NSNumber numberWithDouble:data.acceleration.y],
                        [NSNumber numberWithDouble:data.acceleration.z],
                        nil
                        ];
        [accelerometerDataArray addObject:newData];
    }
}
...
{
    if(!self.catchImage) return;
    NSLog(@"update gyroData %@",data);
    @autoreleasepool {
    NSArray *newData = [NSArray arrayWithObjects:
                        [NSNumber numberWithDouble:data.timestamp],
                        [NSNumber numberWithDouble:data.rotationRate.x],
                        [NSNumber numberWithDouble:data.rotationRate.y],
                        [NSNumber numberWithDouble:data.rotationRate.z],
                        nil
                        ];
    [gyroDataArray addObject:newData];
    }
}
...
- (void)stopRecording
{
	@synchronized( self ) {
        ...
        dispatch_async(_uploadSyncedQueue, ^(){
...
- (void)movieRecorderDidFinishRecording:(MovieRecorder *)recorder
{
    ...
    @synchronized( self ) {
        ...
        [self transitionToRecordingStatus:VideoSnakeRecordingStatusIdle error:nil];
    }
}
